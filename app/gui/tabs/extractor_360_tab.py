...
class Extractor360Tab(QWidget):
    ...
    def set_params(self, params):
        ...
        if "enabled" in params:
            self.check_activate.setChecked(params["enabled"])
        if "interval" in params:
            self.spin_interval.setValue(params["interval"])
        if "resolution" in params:
            self.spin_res.setValue(params["resolution"])
        if "camera_count" in params:
            self.spin_cam.setValue(params["camera_count"])
        if "quality" in params:
            self.spin_quality.setValue(params["quality"])
        ...
        if "ai_mask" in params:
            self.check_ai_mask.setChecked(params["ai_mask"])
        if "ai_skip" in params:
            self.check_ai_skip.setChecked(params["ai_skip"])
        if "adaptive" in params:
            self.check_adaptive.setChecked(params["adaptive"])
        if "motion_threshold" in params:
            self.spin_threshold.setValue(params["motion_threshold"])
