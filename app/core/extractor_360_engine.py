...
class Extractor360Engine(BaseEngine):
    ...
    def run_extraction(self, input_path, output_dir, params, progress_callback=None, log_callback=None, status_callback=None, check_cancel_callback=None):
        ...
        if not self.is_installed():
            if log_callback:
                log_callback("Error: 360Extractor not installed.")
            return False
        ...
        if safe_in is None:
            if log_callback:
                log_callback(f"SECURITY: Invalid input path: {input_path}")
            return False
        if safe_out is None:
            if log_callback:
                log_callback(f"SECURITY: Invalid output directory: {output_dir}")
            return False
