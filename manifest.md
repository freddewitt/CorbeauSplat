...
> Version 2.0.0-rc1 — macOS Apple Silicon Gaussian Splatting Pipeline
...
| **2.0.0-beta.5** (2026-09-20) | **Plan d'audit 8 lots terminé.** Sécurité : `ml-sharp`/`nerfstudio`/`360Extractor` étaient clonés, **compilés et exécutés** depuis une branche non épinglée → `pinned_ref` ; modèles Upscayl vérifiés **fail-closed** sur un catalogue épinglé (2 des 6 s'installaient sans contrôle) ; `linux_brush` renseigné. Nouveautés : **sélection de plage vidéo** (aperçu FFmpeg, timecodes saisis), widget de format de conversion, `pipeline --clean/--export`, dépendances manquantes signalées au démarrage de la GUI. Correctifs : `is_safe_path()` ne vérifiait pas ce qu'elle annonçait et 3 tests de sécurité ne pouvaient pas échouer ; Sharp lancé avec le mauvais Python ; Brush lancé avec un drapeau qu'il rejette ; checkpoints CLI/GUI à des endroits différents ; upscale x1/x2 fabriquait un nom de modèle inexistant ; recharger une configuration pouvait déclencher un téléchargement ; format source illisible signalé « Chemins manquants ». Bouton Lancer de l'onglet Entraînement **retiré** (ajouté en beta.4). Conversion FR→EN terminée. **739 tests**, couverture 45 %. |
...
_Historique 2026-09-20 (journal réparé, chargement de configuration sans téléchargement) et versions ≤ beta.3 : `docs/manifest-detail.md`. Versions ≤ 1.0.6 : `CHANGELOG.md` (historique complet)._
