...
sessions of work that had been orphaned by a Syncthing conflict are back in the
branch. Most of what follows are contracts the code was not keeping.

### 🔒 Security
- **Third-party repositories were cloned, compiled and executed off an unpinned default branch.** `ml-sharp`, `nerfstudio` and `360Extractor` were fetched with `reset --hard FETCH_HEAD`, so an install done today and one done tomorrow ran different code, and an upstream regression — or a compromise — propagated straight onto the machine. `EngineDependency` takes a `pinned_ref` that `update_git()` checks out: nerfstudio `v1.1.5`, 360Extractor `v4.0.1`, and ml-sharp at commit `aed6527` since that repository publishes no tags at all. Bumping a pin is now a deliberate, reviewable act.
- **Upscayl model downloads were verified only when a hash happened to be declared.** Two of the six models carried none, and that branch logged a green tick and installed the file unverified — the check was decorative exactly where it mattered. All ten hashes are filled in and verified against a pinned catalogue commit (the catalogue pointed at a moving `main` branch, so bytes could change while the declared hashes stood still), and verification is now fail-closed: no declared hash, no download. Transfers stream instead of holding ~70 MB in memory twice.
- **`linux_brush` was an empty string in `checksums.json`**, which fails `verify_download_strict` closed on Linux. Filled from upstream's published `sha256.sum` for v0.3.0 rather than by re-hashing a download, which would only attest to what the server served.
- **The SuperSplat viewer listened on every network interface.** `npx serve -p <port>` binds `0.0.0.0`, so anything on the local network could load the viewer. It now starts with `-l tcp://127.0.0.1:<port>`.
- **A full-code audit (36 findings, 15 verified, none critical or high) led to 15 fixes**, mostly hardening: pip-installed engines no longer record the moving remote `HEAD` as their version, Upscayl is pinned to the checksum-verified release, Brush on Windows is refused explicitly instead of failing a checksum, Pillow's decompression-bomb guard is restored (bounded instead of `MAX_IMAGE_PIXELS = None`), an installer failure no longer aborts the remaining engines, and the audit tooling itself refuses to run build recipes controlled by the audited repository.

### ✨ Added
- **The Source panel's mode selector now actually routes to Sharp and 4DGS.** `plan_pipeline()` already shaped the plan per mode, but the plan's `reconstruction` step always built a `ColmapWorker` — so picking Sharp or 4DGS in the dropdown and hitting Lancer ran COLMAP anyway, and both engines were reachable only through their own OUTILS panels. `StudioWindow._build_reconstruction_worker()` now dispatches on the mode recorded at launch: Gsplat → COLMAP, Sharp → `SharpWorker`, 4DGS → `FourDGSWorker`. Sharp and 4DGS take their paths from the Source panel while their own panels keep supplying their settings; 4DGS still stops at dataset preparation, since no Apple Silicon 4D trainer exists.
- **4DGS: the COLMAP settings `run_colmap()` already accepted are now reachable.** `camera_model`, `single_camera`, `matcher_type` and `sequential_overlap` were parameters nothing ever built, so their defaults were the only values obtainable from the UI. The 4DGS panel exposes them (reusing the Reconstruction panel's i18n keys), with the overlap field greyed out unless the sequential matcher is selected; the matcher list is limited to the two modes `run_colmap()` actually implements, `vocab_tree` being silently equivalent to `exhaustive` there.
- **Upscale: the model gallery is back, in the panel's right pane.** The UI redesign left only a dropdown marking installed models with a tick — nothing described a model, showed its weight on disk, or allowed deleting one once downloaded, and the right pane was an empty placeholder. `ModelCard` returns (name, description, scale badge, status, Download/Delete button), sharing the dropdown's one-download-at-a-time guard so a card click and a dropdown change can't start two transfers at once. `UpscaylModel.size_on_disk_mb()` regained alongside it.
...
- **Pick an in/out range on a single video before extraction.** A "Sélection vidéo…" button in the Source panel, enabled only for one video file and greyed out for a folder — several videos mean several timelines, which one in/out pair cannot describe. The preview is built from ffmpeg stills rather than QtMultimedia: ffmpeg is already a hard dependency and decodes every container in `VIDEO_EXTENSIONS`, while Qt's backend fails silently on several of them (`.mts`, `.insv`, `.mxf`). Timecodes can also be typed, in any form the interface itself displays. The range reaches the extraction as `-ss` plus `-t` (a duration, not `-to`, which after a pre-input seek would be counted from the seek point and silently cut the wrong span). Outside `.mp4`/`.mov` the dialog advises converting, since those containers' keyframe layout makes a mark land further from what the preview showed — advice, not a block. CLI parity: `--trim_start`, `--trim_end`.
- **The image conversion format is selectable in the Source panel.** `--convert png|jpeg|off` had existed since the video/HEIC work but the interface was pinned to `png`, so a GUI user could neither choose JPEG nor deliberately pass originals through untouched. This was the last known CLI/GUI divergence.
- **`pipeline` runs the chain its name promises.** It stopped after Brush; `--clean [light|medium|strong]` and `--export FORMAT` (with `--export_output`) continue into Cleaning and Export, and the step headers count the real total instead of always claiming "1/2".
- **Missing dependencies are reported at GUI startup**, which never ran the check at all. Two tiers: core tools (ffmpeg, COLMAP, send2trash) raise a dialog, per-feature ones (Glomap, Upscayl, Brush, npx) go to the log, since they only matter to someone about to use that feature. `check_dependencies()` now covers all of them, each labelled with what it blocks.
- **`--sequential_overlap` and `--guided_matching` exist as CLI flags.** `_build_colmap_params` read them through `getattr`, so a missing flag produced no error — just a value the user could never change. `guided_matching` was exposed in the GUI and consumed by COLMAP; only the CLI ignored it.

### 🐛 Fixes
- **4DGS: the nerfstudio path fed SfM every frame of every camera, like `run_colmap()` used to.** The beta.4 fix staged one synchronized frame per camera before COLMAP, but only on the manual path; `process_dataset()` still handed `ns-process-data` the whole `images/` tree. Since `ns-process-data` runs COLMAP internally, it hit the exact same static-scene violation — and it expects a flat folder anyway, so it would not have descended into the `cam_XX` subfolders. Both branches now run on the same `_build_static_reference_set()` staging folder.
- **SuperSplat offered an update at nearly every startup.** The installer reset to `FETCH_HEAD` of PlayCanvas' default branch and saved the resulting commit SHA as the local version, while the base class compared it against the remote HEAD — so any commit pushed upstream made the two differ. `SUPERSPLAT_PINNED_TAG` (currently `v3.3.0`) now drives both what gets checked out and what the comparison sees; a newer upstream tag is merely reported, never adopted on its own. Same reasoning as the Brush release pin, minus `checksums.json`: SuperSplat is built from source, so there's no archive hash to keep in step.
- **`is_safe_path()` promised a containment check it never performed.** It resolved the path and tested existence, so a caller trusting the name believed it filtered directory traversal. It had no application caller at all. Renamed `path_exists()`, with the contract stated plainly; the dead `project_root` parameter of `validate_path_standalone()` is gone. No containment was added on purpose: users pick sources and outputs anywhere, so enforcing a root would reject legitimate input.
- **Three path security tests could not fail.** Two passed only because the paths did not exist, not because anything rejected them, and the third ended on `assert result is None or isinstance(result, Path)`. They would have stayed green with all validation removed. Rewritten against an existing target, so the real behaviour — `..` is normalised, not refused — is visible.
- **`delete_project_content()` documented a whitelist over a blacklist body.** The docstring claimed deletion was allowed "only if contained within project_root or user home directory"; the guard refuses catastrophic paths and accepts everything else, which is intended — projects live on external volumes. Docstring corrected and the real contract pinned by a test.
- **`ExportEngine.is_available()` returned a hardcoded `True`,** so a missing GLB or SPZ backend surfaced only as a failed export after the wait. It probes per format now, checked at the top of `export()`.
- **Sharp could be launched with the wrong interpreter.** `_get_sharp_cmd()`'s last resort was `[sys.executable, "-m", "sharp.cli"]` — the main venv's 3.13, while Sharp requires 3.11 in `.venv_sharp` — so it could only raise a bare `ModuleNotFoundError` mid-run. Removed; `is_installed()` is now defined as "that function found something runnable", so the two can no longer disagree.
- **A source in an unreadable format reported "Chemins manquants".** `detect_source_kind()` collapses every failure into "empty", so pointing the app at a folder of camera RAW or PSD — a perfectly valid path — sent the user looking in the wrong place. The message now names the formats actually found, lists the accepted ones, and surfaces the conversion setting that makes that list wider than JPEG/PNG.
- **Brush could be started with a flag it rejects.** `get_brush_build_mode()` fell back to "release" whenever `engines/brush.version` was missing, and a source build then received `--total-steps` instead of `--total-train-iters`, killing the training immediately. It now asks `brush --help` which flag exists, caching the answer. Also fixes a `TypeError` when `checkpoint_interval` is present as `None`.
- **Upscale at x1 or x2 produced a model name that does not exist.** A substitution rewrote `realesrgan-x4plus` into `realesrgan-2plus`, and the manager then refused with "Modèle introuvable". Removed: ncnn models have a fixed native scale and `-s` is what resizes. CLI and GUI now offer the same 1–4; x1 is kept because it converts formats without upscaling.
- **CLI and GUI put Brush checkpoints in different places.** The CLI passed the dataset folder as Brush's output while the GUI applies `<output>/<project>/checkpoints`, so a project trained in one and reopened in the other could not find its checkpoints. Both call `resolve_checkpoints_dir()` now.
- **Two COLMAP flags could not be turned on.** `--view-graph-calibration` and `--ignore-watermarks` were declared `store_true` with `default=True`, making the positive form a no-op and letting the hand-written `--no-*` twins drift. Both use `argparse.BooleanOptionalAction`.
- **The SuperSplat data server named itself as its own allowed CORS origin** (the header was built from its port, not the viewer's), and `self.httpd` was assigned inside the worker thread, leaving a window where `stop_data_server()` read `None` and returned while a server kept running unreferenced. The socket is bound in the calling thread now, and `dist/` and `npx` are checked before spawning.
- **Reloading a saved configuration could start a network download.** `UpscalePanel._on_model_changed()` was wired to `currentIndexChanged`, which fires on `setCurrentIndex()` too — the call `set_state()` makes. It listens on `activated` now, so only a deliberate choice fetches; a restored configuration reports "model not installed" instead of quietly transferring.
- **Six records in `journal.jsonl` had been invalid JSON for six weeks.** Traced to the `8e7af63` bulk re-import of 2026-08-08, not to the journal-writing code: each had lost an array key and its opening bracket, and nothing ever crashed because the file is read line by line. Repaired with every character preserved, and `tests/test_journal_integrity.py` now guards it.
- **Upscale silently overwrote images sharing a base name.** Staged files were named `<stem>_<model>_x<scale>.<fmt>` without the source extension, so `a.jpg` and `a.png` in one folder collided and only one result survived. Colliding sources now also carry their original extension; every other name is unchanged.
- **Audit fixes that protect user data and keep the interface responsive.** A pre-existing `temp_frames` or `colmap_reference_frames` folder is no longer deleted; a NaN splat scale no longer empties the whole cleaned cloud; resuming after a partial failure no longer skips the upscale; closing the window waits for the running worker instead of destroying a live `QThread`; and the 360, COLMAP and splat-transform workers always emit `finished_signal` even when the engine raises, so the interface no longer freezes.
...
- **The Rosetta 2 warning is visible.** It went through `warnings.warn`, which writes to stderr — invisible when the app is launched from its icon, which is precisely the person it targets: someone losing 20–40% on COLMAP, Brush and upscayl without any sign. `rosetta_warning()` returns the text so each front end places it where its users look; the GUI puts it on the first line of the run log.
- **`BrushWorker.run()` went from 153 lines to 36.** It mixed dataset resolution, Refine environment setup, checkpoint archiving, process launch and result handling inside a `QThread`. Split into named steps with error handling unchanged — which made the Refine path testable for the first time.
- **`apply_source_blur_settings()` is `apply_source_settings()`**, carrying the conversion format as well as the blur fields.
- **`is_checkpoint_ply()` and `find_checkpoint_plys()` moved** from `workers.py` to `chaining_logic.py`, which is deliberately Qt-free, so the CLI can apply the same rule instead of a second copy.
- **French comments are gone.** The conversion finished: a sweep over `app/` and `tests/` reports zero remaining occurrences. The historical "~76 comments" figure was a large underestimate — the audit counted 598 across 66 files.
- `requires-python` gains the `<3.14` bound the launcher already enforces, and `AGENTS.md`/`manifest.md` no longer claim PyQt6: no file uses it, everything is PySide6.

### 🌍 i18n
- New key `btn_view_logs` (error dialog's log button). All 9 locales updated.
- New keys `up_models`, `up_download`, `up_delete`, `up_downloading`, `up_not_installed`, `up_bundled`, `up_delete_title`, `up_delete_body` (Upscale model gallery). All 9 locales updated.
- New keys for the video range dialog (13), the Source panel's conversion selector (5), the SuperSplat server messages (9), the missing-dependency dialog (3), and the Upscale "model not installed" status (1). All 9 locales updated.
- Locale keys must be literal, never built with an f-string: `test_no_orphan_locale_keys` scans `tr()` call sites statically.
...
- **The Training tab's local Launch button, added in beta.4, is gone.** That tab has no manual path fields, so the button had to infer them, and an early version inferred them wrongly and moved a user's whole project folder through the checkpoint archiving logic. It remains in the OUTILS "Brush" module, where the paths are typed by hand. Training inside the chain starts from the single Launch button in Source, or automatically after Reconstruction when "Lancer Brush" is ticked.

### 🧪 Tests
- **854 pass**, up from 471. ruff and mypy clean across 80 files.
- Guards added against the drift this release fixed: every `ColmapParams` field must have a CLI flag (25 fields, no exemptions), `manifest.md` is pinned against the CLI dispatch table, and `journal.jsonl` must stay parseable.
- Panels are now exercised against a **real Qt in a subprocess**, since the suite's PySide6 mock is process-global and irreversible. That covers what the AST contracts cannot: that a setting survives being saved and reloaded.
...
### 🚀 Startup & packaging
- **Brush would no longer install: the pinned checksum was stale.** `checksums.json` pinned `darwin_brush` to a hash from an earlier release, so the fail-closed `verify_download_strict()` guard (added by audit A2-C1) correctly refused to install v0.3.0 — reported as `SHA256 mismatch or missing reference hash`, followed by a misleading `Vérifiez votre connexion`. Re-pinned to `65b26313…a1048c`, the v0.3.0 macOS/arm64 digest, cross-checked against **three independent sources**: the GitHub API `digest` field, upstream's `<asset>.sha256` sidecar, and upstream's release-wide `sha256.sum`. The asset's `updated_at` equals its `published_at` (2025-09-14), confirming a stale pin rather than a re-uploaded artifact. The hash was deliberately *not* recomputed from the freshly downloaded archive — that would only attest to what the server just served, defeating the guard. `_instructions` now documents where to source upstream digests, and notes that the pin must be refreshed whenever `get_remote_version()` resolves to a newer tag.
- **The Brush release is now pinned, not tracked.** The stale hash above was a symptom; the cause was structural. `install()` resolved the *latest* upstream tag via `get_remote_version()` while `_install_from_release()` verified the archive against a hash frozen for one specific version — guaranteeing a mismatch the moment upstream published anything. Worse, `base.py` compares that same value to the local version, so every startup would have offered an update leading to a certain failure. `checksums.json` gained a `brush_release` key next to the digest; `get_remote_version()` returns it in release mode. The upstream tag is still fetched, but purely to *inform* (`ℹ️ Brush vX available upstream; this repo pins vY`, with the URL of upstream's `sha256.sum`). Adopting a new release is now a deliberate, reviewable edit that moves version and digest together. The hardcoded `or "v0.3.0"` fallback in `install()` was removed — a third home for the version meant a third way to drift.
  - Deliberately **not** implemented: fetching upstream's `<asset>.sha256` sidecar to verify the download. It reads like the natural fix, but the sidecar ships from the same origin as the artifact it vouches for — anyone able to swap one can swap the other. It would have traded a real guarantee for convenience. Pinning keeps this repository's own history as the trust root.
- **The startup installer announced success after a failure.** `base.py` only caught *exceptions*, but `install()` reports failures through a return value (`BrushInstaller._install_from_release` prints its error and returns `False`). The log therefore showed `❌ Téléchargement release échoué`, then `✅ Brush installed automatically`, then `❌ Brush: Missing` — two contradictory lines. Success is now printed only when `engine.is_installed()` holds afterwards, and `on_startup_ready()` no longer runs after a failed install.
- **`--clean` left `.venv_4dgs` behind** while claiming a complete reset. The project has four virtualenvs; the launcher purged three. The heaviest one (nerfstudio) survived exactly the reset meant to reclaim it. Both the `rm -rf` and the warning message now list all four.
- **`trimesh` was installed behind the lockfile's back.** The launcher ran four bare `pip install <pkg>` calls (PySide6, send2trash, plyfile, trimesh) whenever an import failed. Three of those are in `requirements.lock` — so if the fallback ever fired, it silently installed an *unpinned* version over the pinned one. `trimesh` was in no dependency file at all: that ad-hoc call was the only way it reached the venv, invisible to anyone reading the lock. It is now declared in `requirements.txt`, `requirements.lock` (`==5.0.0`) and `pyproject.toml`, and removed from deptry's `DEP001` ignore list. The four installs are replaced by a single check that *verifies* and exits with a repair command, without installing anything; `✅ Dépendances synchronisées et vérifiées` now comes after that check rather than before it.
- **The four deptry findings are resolved, and the tool is now wired into CI** so they cannot accumulate again. `requests` and `urllib3` (`DEP002`) were declared runtime dependencies that **no file imports** — all networking goes through `urllib.request`, the standard library module, not `urllib3`. Confirmed by grep and by `pip show`: within the runtime set nothing requires them; they only remain installed in dev environments, pulled in by `pip-audit` via `CacheControl`. Removed from `requirements.txt`, `requirements.lock` and `pyproject.toml`, closing audit item **A2-M14**. `spz` (`DEP003` ×2) is compiled and installed into the main venv by its own installer, never by pip from a requirements file — so deptry sees it as "installed but undeclared" when present and "undeclared" otherwise. It is now ignored under `DEP003` as well as `DEP001`, along with `open3d` and `bpy`: both lists cover the same set, so the result no longer depends on the state of the venv. Verified under both conditions — with the venv populated, and in an isolated environment reproducing CI, where the runtime dependencies are absent.
- **deptry was configured but never runnable**: `[tool.deptry]` has existed in `pyproject.toml` for a while, yet the tool was absent from `requirements-dev.txt`, the `dev` extra, and CI — so it had never been executed. Added to all three declaration sites (and to its own `DEP002` ignore list). Its first run confirms `trimesh` is now clean and surfaces four pre-existing findings, left untouched and recorded in `manifest.md`.

### 🔁 UI restructuring (Studio) — Complete Refonte
- **Left rail reorganized into three tiers**: **Projet** (top, always visible), **PARAMÈTRES** (5 pipeline steps: Reconstruction, Entraînement, Nettoyage, Export, Visualiser — expanded by default), **OUTILS** (7 modules collapsed by default). Static hierarchy with all 13 items permanently visible; no more collapsible sections.
- **TopBar removed** (`app/gui/topbar.py` deleted): single source of truth for project name moved to Projet panel. Pipeline mode selector and Settings button relocated. Minimal app bar now shows icon + version only.
- **Settings integrated into Paramètres panels**: right sidebar now displays panel-specific options instead of shared Settings window. Empty-right convention applied uniformly.
- **Primary action styling**: Launch button now prominent (filled accent) with dynamic theme support via Qt properties; destructive "Delete dataset" button clearly separated.
- **Live Upscale & 360 Extractor**: Both promoted from OUTILS to PARAMÈTRES pipeline steps, participating in rail status tracking (DONE/ERROR). Dual access maintained (standalone in OUTILS, chained in PARAMÈTRES via `upscaler_avant` / `source_360` flags).

- **Upscale left OUTILS and became a first-class pipeline step**, first in the PARAMÈTRES group (it runs *before* Reconstruction, so it leads the list). `PIPELINE_STEPS` gained `"upscale"` between `source` and `reconstruction`, `TOOL_KEYS` dropped from 7 to 6 entries, and the `⤢` glyph moved from `_TOOL_ICONS` to `_STEP_ICONS`. Its i18n key was renamed `rail_tool_upscale` → `rail_step_upscale` in all 9 locales. The panel keeps a single instance (unlike Entraînement/Brush or Visualiser/SuperSplat, which are deliberately twinned) and its `_build_right()` now returns a bare `QWidget()`, all settings having moved to the centre column as on the other Paramètres steps.
- **Real chaining for Upscale**, driven by a new `upscaler_avant` flag (default **off**, unlike "Lancer Brush") exposed as the first checkbox of the Projet panel's Automatisation block (new key `chain_upscale_before`). `plan_pipeline()` inserts `upscale` before `reconstruction` when the flag is set — in every mode, 4DGS included, since 4DGS truncates *after* Reconstruction. Because the project's `images/` folder does not exist yet at that point in the run, the step takes one of two routes: for an **image** source, a new `UpscaleImagesWorker` writes the enlarged images to `<sortie>/<projet>/images_upscaled` and `_build_colmap_worker` feeds COLMAP that folder instead of the raw one — the user's source folder is never touched; for a **video** source there are no frames to enlarge yet, so the step logs `run_upscale_deferred` and hands its parameters to `ColmapWorker`, whose engine already upscales right after frame extraction and before feature extraction (the pre-existing `upscale_config` mechanism in `engine.py`, until now unreachable from the GUI). Both routes therefore enlarge the images before COLMAP reads them. The step participates in the rail's DONE/ERROR status like any other, and the panel's local Lancer button still runs a standalone upscale on the paths typed into it, independently of the chain.
- **Upscale settings are now saved with a named configuration** (`ChainConfig.upscale`, plus `get_state`/`set_state` on `UpscalePanel`); they were the only Paramètres step missing from `collect_config`/`apply_config`. Configurations written before this change reload unaffected — the section simply defaults to empty.
- `TestWorker`'s body was extracted into a module-level `run_upscale_job()` so the standalone and pipeline workers share one upscayl invocation rather than two copies. The class name `TestWorker` is misleading but out of scope here and left untouched.
- Source-type resolution (explicit Images/Vidéo choice vs. `detect_source_kind` auto-detection, with the mixed-content failure) moved into a shared `_resolve_source_type()`, so Upscale and Reconstruction cannot classify the same source differently.

- **360 Extractor became a pipeline step too, while staying in OUTILS** — dual access, the same arrangement as Brush/Entraînement and SuperSplat/Visualiser. `PIPELINE_STEPS` gained `"extraction360"` right after `source` (7 → 8 entries), and it leads the PARAMÈTRES group since it runs before Upscale. `TOOL_KEYS` is unchanged: `"360"` stays, and `self.panels` now holds two independent `Extractor360Panel` instances (`"extraction360"` and `"360"`), each with its own local Lancer button. Driven by a new `source_360` flag (default off), first checkbox of the Projet panel's Automatisation block.
- **The images folder is now propagated between steps through a single channel.** Upscale's `_upscaled_images_dir` was renamed `_pipeline_images_dir` and generalised: every pre-step that produces a new folder overwrites it, and the next step reads it instead of the raw Source path. 360 Extractor does not transform images in place — it *emits* a new folder of planar images from an equirectangular source — so Upscale and COLMAP both consume `<output>/<project>/images_360` when it has run. A second channel would have let the two steps disagree about which folder is current.
- One consequence worth naming: after a 360 extraction the source is always a folder of images, never a video, so Upscale's video branch (which defers the work to COLMAP's engine) is correctly bypassed. The `source_360` + `upscaler_avant` combination therefore runs 360 → upscale → COLMAP with a real worker at each step.
- `Extractor360Panel` moved its `_build_right()` into `_build_center()` like the other Paramètres steps. Since the class is shared by both instances, OUTILS > 360 Extractor inherits the same layout — deliberate, consistent with the choice already made for Brush/SuperSplat. It also gained `get_state`/`set_state` and a `ChainConfig.extraction360` section, so its settings travel with a named configuration.

### 🐛 Fixes
- **Three Projet-panel fields were dead ends**: `checkpoint_dest`, `export_dir` and `export_format` were editable and serialised (`SourcePanel.get_state`/`set_state`, so they even survived a save/reload) but no worker ever read them — filling them in had literally no effect. They are now wired.
  - `checkpoint_dest` reaches `BrushWorker` with its original 1.2.3 semantics restored verbatim: checkpoints go to `<destination>/<project name>/` (no `checkpoints/` subfolder), and `keep_only_latest=True` is passed so intermediate checkpoints are pruned. An empty field preserves the previous behaviour exactly (`<output>/<project>/checkpoints`, all checkpoints kept). `BrushWorker` already accepted `keep_only_latest`; the GUI simply never set it.
  - Critically, `_latest_brush_ply()` — which feeds Nettoyage/Export/Visualiser — now resolves the same folder through the same helper. It previously hardcoded `<output>/<project>/checkpoints`, so the whole post-training chain would have silently found nothing the moment a custom destination was set.
  - `export_dir` and `export_format` override the Export panel during chained runs (they are the *chain's* settings; the panel also serves standalone launches). Left empty, the previous behaviour is unchanged — output falls back to the source PLY's folder, and only when the user hasn't typed one in.
- **Export format list was duplicated**: the Projet panel hardcoded `("spz", "glb", "obj", "ply", "xyz")` while the Export panel read `ExportEngine.SUPPORTED_FORMATS`. Both now read the engine — had the two drifted, `findData` would have returned -1 and the setting would have been dropped without a word.
- New Qt-free `app/gui/chaining_logic.py` holds these rules, so they are testable without instantiating `StudioWindow` (whose Qt base class is a MagicMock under the test harness). Same split as the existing `reconstruction_logic.py`.

- **Deleting a Brush preset and a saved configuration is now possible from the UI.** `delete_user_preset()` and `delete_config()` already existed and were already tested — only the caller was missing, so both were unreachable and the lists could only ever grow.
  - A 🗑 button sits next to the preset dropdown's 💾. It is enabled only for *user* presets: "Défaut" (data `None`) and the built-in `BRUSH_PRESETS` are not deletable, since removing them from the dropdown would have no persistent effect — `merge_presets()` reinjects them on the next reload. The guard is a new pure `brush_presets.is_deletable()` rather than an inline check duplicated at both call sites. A user preset shadowing a built-in of the same name *is* deletable, and the built-in reappears afterwards.
  - A "Supprimer…" button joins Charger/Sauvegarder in the Settings window, emitting a new `deleteRequested` signal handled by `StudioWindow.delete_config_dialog()` — same idiom as `load_config_dialog` (pick from `list_configs()`), plus a confirmation, since the operation removes a file and cannot be undone.
- Both confirmation strings contain a `{0}` placeholder, so they are passed to `tr()` **without** a default: when the key resolves, `LanguageManager.tr()` consumes any extra argument as a `.format()` argument, which would have substituted the French default text into the placeholder. The existing code already documented this trap at one call site; it now bites nowhere.

### ✨ New Features
- **Source type selector** in the Projet panel — Images / Vidéo / **Auto** (default). The FPS field now only appears when the resolved type is a video, and re-evaluates on input-path changes with a 300 ms debounce.
- **Mixed-content detection**: a new pure, Qt-free `detect_source_kind(path) -> "images"|"video"|"mixed"|"empty"` in `reconstruction_logic.py` replaces `studio_window._detect_input_type`, which silently ignored folders containing both images and videos. A warning banner now appears in the panel, and a launch on mixed content fails cleanly instead of feeding COLMAP an arbitrary guess.
- **Real post-training chaining**: the Nettoyage → Export → Visualiser toggles previously only affected the breadcrumb — the chain stopped after Entraînement and asked the user to continue manually. Each enabled step now runs for real inside the existing `_run_pipeline_step` walk (no separate `PostTrainingWorker`), pre-filled from the previous step's output and reported in the rail as DONE/ERROR. Visualiser, which runs a persistent server rather than a worker, is marked DONE as soon as the server starts.
- **Cancel button in the log bar**, enabled only while a worker is running.
- **Launch button on the Reconstruction panel**, for a standalone COLMAP run outside the chain.
- **Brush enabled by default** in the Projet panel's chaining toggles.

### 🐛 Fixes
- **Language/theme silently changed by scrolling** (`settings_window.py`): `_NoScrollComboBox.wheelEvent` only ignored the wheel when the widget lacked focus — but clicking the combo to open it *gives* it focus, so any subsequent scroll changed **and persisted** the language or theme. The wheel is now ignored unconditionally; selection requires clicking an entry in the dropdown.
- **4DGS scope was undocumented in the UI**: the module only prepares the dataset (multi-camera extraction + COLMAP reconstruction in Nerfstudio format) — no native Apple Silicon 4D Gaussian Splatting trainer exists (nerfstudio/gsplat are CUDA-only). A permanent banner now states this in the panel, and the `4dgs` entry of the mode dropdown carries a matching tooltip.
- **`refine_mode` regression restored**: `params["refine_mode"] = panel.combo_mode.currentData() == "refine"` had been dropped from `_build_brush_worker` and `_launch_brush` in the working tree, disconnecting the Refine dropdown from the engine. Restored in both places.

### 🌍 i18n
- 10 new keys across all 9 locales (`rail_section_parametres`, `rail_step_projet`, `source_type_label`, `source_type_auto`, `source_type_images`, `source_type_video`, `err_source_mixed_types`, `err_visualiser_start`, `fourdgs_mode_tooltip`, `fourdgs_scope_notice`), 3 orphans removed (`rail_section_pipeline`, `rail_step_source`, `topbar_project`). 265 → 272 keys, the 9 locales remain key-aligned.
- Note: `test_locales.py` only detects literal `tr("...")` calls, so the 5 keys reached through tuple/dict indirection (`tr(*key_default)` in `rail.py`, and the source-type table in `source_panel.py`) were **not** flagged as missing — they would have silently stayed French in the other 8 languages. Worth tightening that test.
